...
def raw_audio_to_spectograms(raw_audio: np.array, keyframe_timestamps: list[int], location: str, frame_rate:int=48000, window_size_ms:int = 1000):
    margin = int((window_size_ms/2000)*frame_rate) # Margin is 1/2 window. Framerate per second, window size in ms. 
    ...
    for keyframe in keyframe_timestamps:
        spectogram = raw_audio_to_spectrogram(     #TODO: edge case if keyframe is very close to start/end video   
                    raw_audio[:, keyframe*frame_rate-margin:keyframe*frame_rate+margin] ) 
        np.array(spectogram).tofile(os.path.join(location,f'{keyframe}.npz'))    

def extract_audio_spectograms(media_file: str,keyframe_timestamps: list[int],location: str, tmp_location: str):
